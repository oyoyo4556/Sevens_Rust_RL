...
impl Processor {
    ...
    pub fn new(max_batch_size:usize) -> Self {
        Self {
            states_buf:RefCell::new(Vec::with_capacity(max_batch_size*233)),
            next_states_buf:RefCell::new(Vec::with_capacity(max_batch_size*233)),
            next_masks_buf:RefCell::new(Vec::with_capacity(max_batch_size*53)),
            infer_buf:RefCell::new(Vec::with_capacity(282)),
        }
    }
    ...
    -> Result<(Tensor,Tensor,Tensor,Tensor,Tensor,Tensor,Tensor)> {
        ...
        let states = Tensor::from_slice(&s_buf,(batch_size,233),device)?;
        let next_states = Tensor::from_slice(&ns_buf,(batch_size,233),device)?;
        ...
        Ok((states,next_states,next_masks,actions,rewards,dones,next_gammas))
