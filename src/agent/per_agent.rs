...
impl PERDQNAgent {
    pub fn new(capacity:usize,n_step:usize) -> Self{
        ...
        Self { 
            ...
            buffer: PrioritizedReplayBuffer::new(capacity,0.4,0.4,0.000002),
            gamma: 0.99,
            epsilon: 0.8,
            epsilon_min: 0.01,
    ...
    pub fn update(&mut self,batch_size:usize) -> Result<f32> {
        ...
        let (batch,idx,is_weight) = self.buffer.sample(batch_size) ;
        let is_weight_t = self.processor.is_weight_to_tensors(is_weight, &self.device)?;
        ...
        //priority_update
        let td_errors_t = (&current_q - &target_q)?.abs()?;
        let td_errors : Vec<f32> = td_errors_t.to_vec1()?;
        self.buffer.update_priorities(&idx, &td_errors);

        //main_loss
        let huber_loss = candle_nn::loss::huber(&current_q,&target_q,1.0)?;
        let loss = huber_loss.broadcast_mul(&is_weight_t)?.mean_all()?;
    ...
    pub fn copy_weights_to(&self,other:&mut PERDQNAgent) -> Result<()> {
